...
export default function PostComponent({posts}:{posts:any}) {
    return (
        <div>
             {
                posts.map((index:any, key:any) => (
                    <div key= {index.id} className="card box-border border-2 rounded-lg" style={{ width: '77%' }}>
                        <Link to="/detail"><h2 className="font-bold">{index.title} - {index.id}</h2></Link>
                        <div className='flex '>
